...
button.addEventListener('click', function(event){
    ...
    if(guess.length===5) compareTarget(guess);
});

function compareTarget(guess){
    let greens=0;
    let id = "row"+count;
    let tds = document.getElementById(id).children;
    for(let td of tds){
        let idx = Number(td.id);
        td.innerText = guess[idx];
        if(guess[idx]===word[idx]){
            td.style.backgroundColor = "green";
            td.style.color = "white";
            greens++;
        }
        else if(word.includes(guess[idx])){
...
function endGame(greens){
    let p = document.getElementById("congrats");
    if(greens===5){
        p.textContent = "Congratulations! You guessed the word!";
    }
    else{
        p.textContent = `Game over, the word was "${word}"`;
    }
    let input = document.getElementById("guess");
    input.disabled = true;
}